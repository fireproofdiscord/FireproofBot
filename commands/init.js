	...
	const req = https.request(options, async res => {
		if (res.statusCode === 200) {
			try {
				...
				await promises.writeFile(`../FireproofRepos/${guildId}/roles.sys.json`, JSON.stringify(roleData, null, 4))

				for (let [path, channel] of Object.entries(categoryData)) {
					await promises.writeFile(`../FireproofRepos/${guildId}/${path}/category.sys.json`, JSON.stringify(channel, null, 4));
				}

				for (let [path, channel] of Object.entries(channelData)) {
					await promises.writeFile(`../FireproofRepos/${guildId}/${path}.json`, JSON.stringify(channel, null, 4));
				}
				...
				await interaction.followUp(`Repository created at https://bitbucket.org/fireproofdiscord/${slug}`);
			} catch (err) {
				await promises.rm(`../FireproofRepos/${guildId}`, { recursive: true });
